...
#define PBC_PATH (libpbc_config_getstring(p, "pbc_path", "/usr/pubcookie/"))
/* the login server builds it's key Filenames from the hostname     */
#if defined (WIN32)
#  define PBC_KEY_DIR (AddSystemRoot("\\inetsrv\\pubcookie\\keys"))
#else
#  define PBC_KEY_DIR (libpbc_config_getstring(p, "keydir", "/usr/pubcookie/keys"))
#endif

/* where the runtime configuration file lives */
#define PBC_CONFIG "/usr/pubcookie/config"

/* Where to find templates */
#define TMPL_FNAME (libpbc_config_getstring(p, "template_root", "/usr/pubcookie/login_templates"))
...
#define OK_BROWSERS_FILE (libpbc_config_getstring(p, "ok_browsers", "/usr/pubcookie/ok_browsers"))

/* file to get browser information from */
#define BROWSERS_FILE (libpbc_config_getstring(p, "browsers", "/usr/pubcookie/browsers"))

/* getcred authz file */
#define GETCRED_AUTHZ (libpbc_config_getstring(p, "getcred_authz_file", "/usr/pubcookie/getcred_authz"))

/* The keyclient.  This is probably better defined elsewhere, but I dunno. */

#define KEYCLIENT (libpbc_config_getstring(p, "keyclient", "/usr/pubcookie/keyclient"))

/* The path to the cert db, if this is using the Netscape SDK */

#define CERT_DB_PATH (libpbc_config_getstring(p, "cert_db_path", "/usr/pubcookie/keys"))
