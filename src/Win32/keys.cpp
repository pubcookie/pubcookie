...
typedef int (*GETINBOUNDBYTE)(PPOSDATA p);
	...
	// LISTPROP struct is used to maintain a set of list-wide properties.  This implementation
	// uses the properties list to hold a buffer pointer.
	...
	// This LISTPROP struct holds temporary file information.
...
//	Returns:
//
//		incoming byte value or -1 to indicate an error
...
int GetQueryByte(IN OUT PPOSDATA pPosData)
{
    // For readability only...
    EXTENSION_CONTROL_BLOCK *pECB;
...
//	Returns:
//
//		binary value of the character (0-15) -1 if the character is not hex digit
